...
export class SelectSpecilityPage implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe((params) => {
      this.location_id =
        this.router.getCurrentNavigation().extras.state.location_id;
      this.location_name =
  ...
  getSpeciality() {
    this.http
      .getSpeciality("getSpecialities/location/" + this.location_id)
      .subscribe(
        (res: any) => {
          if (res.success) {
            let data = res.data;
            ...
            data.map((spec) => {
              data2.map((allSpec) => {
                if (spec.speciality_id == allSpec.id) {
                  newArr.push(spec);
                }
              });
            });
            this.specialities = newArr;
            this.searchArray = this.specialities;
            if (this.specialities.length == 0) {
              this.utility.showMessageAlert(
                "No Speciality!",
                "This location has no speciality added."
              );
            }
          } else {
            this.utility.showMessageAlert(
  ...
  selectSpeciality(speciality_name: any, id: any) {
    const navigationExtras: NavigationExtras = {
      state: {
  ...
  search(searchInput: { target: { value: any } }) {
    if (searchInput.target.value != null) {
      // tslint:disable-next-line: only-arrow-functions
      this.specialities = this.searchArray.filter(function (
        ele: { speciality_name: string },
        i: any,
        array: any
      ) {
        const arrayelement = ele.speciality_name.toLowerCase();
