...
export class ChatWithDoctorPage implements OnInit {
  ...
  constructor(
    ...
    this.patient_profile_image =
      user.profile_photo != undefined ? user.profile_photo : "";
  ...
  addPatient() {
    ...
    if (
      ...
      this.utility.showMessageAlert(
        ...
        "Only alphabets are allowed to enter in patient name field."
      );
      ...
      this.utility.showMessageAlert(
        "Invalid age !",
        "The age  you have entered is not valid."
      );
    } else {
      this.show_patient_form = false;
      this.book_for = "relative";
  ...
  payNow() {
    ...
    if (this.doctor_id == undefined || this.doctor_id == "") {
      this.utility.showMessageAlert(
        "Doctor is required!",
        "Please choose one of our doctors whom you want to chat with."
      );
    } else if (this.health_query == undefined || this.health_query == "") {
      this.utility.showMessageAlert(
        "Health query is missing!",
        "Please write about your query you want to ask."
      );
    } else {
