...
export class EditProfilePage implements OnInit {
  ...
  constructor(
    ...
    this.country_status = user.country_status;
    this.gaurdian_name = user.guardian_name == null ? "" : user.guardian_name;
    this.emergency_number =
      user.emergency_num == null ? "" : user.emergency_num;
    this.marital_status =
      user.marital_status == null ? "" : user.marital_status;
  }
  ...
  pickCountry(ev) {
    this.country_status = ev.detail.value;
  }
  editProfile() {
    if (this.gaurdian_name.length > 25 && this.gaurdian_name != undefined) {
      this.utility.showMessageAlert(
        "Invalid guardian name!",
        "Guardian name should not be more than 25 characters."
      );
    } else if (
      this.emergency_number.length != 10 &&
      this.emergency_number != ""
    ) {
      this.utility.showMessageAlert(
    ...
    } else if (this.country_status == "") {
      this.utility.showMessageAlert(
        "Country is mandatory!!",
        "Please select country you belong"
      );
    } else if (this.name == "") {
      this.utility.showMessageAlert(
      ...
      this.http.editProfile("editProfile", params).subscribe(
        (res: any) => {
          ...
          if (res.success) {
            res.data.profile_update = "1";

            localStorage.setItem("user_details", JSON.stringify(res.data));
            this.utility.user = JSON.parse(
              localStorage.getItem("user_details")
            );
            this.utility.showMessageAlert(
              "Account Updated!",
              "Your new info has been added."
            );
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          text: "Remove",
          handler: () => {
            this.http.deleteReport("removeProfile", parms).subscribe(
              (res: any) => {
